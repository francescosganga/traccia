...
- `src/renderer` — React UI. Four windows: `index.html` (main), `region.html` (drag-to-select overlay), `controls.html` (floating widget, excluded from the capture), `frame.html` (outline of the region being recorded: click-through, excluded from the capture). `src/components/Icon.tsx` is the icon set.
