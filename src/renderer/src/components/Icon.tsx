...
const PATHS: Record<IconName, ReactElement> = {
  record: (
    <>
      ...
      <circle className="fill" cx="12" cy="12" r="3.5" />
    </>
  ),
  settings: (
    <>
  ...
  more: (
    <>
      ...
      <circle className="fill" cx="19" cy="12" r="1.6" />
    </>
  )
}
...
/** The app mark: the same ring + dot as the app icon, on a rounded dark tile. */
export function Logo({ size = 20 }: { size?: number }) {
  return (
    <svg className="logo" width={size} height={size} viewBox="0 0 24 24" aria-hidden="true">
      <rect width="24" height="24" rx="6" fill="var(--bg-elev-2)" />
      <circle cx="12" cy="12" r="6.5" fill="none" stroke="var(--rec)" strokeWidth="1.6" />
      <circle cx="12" cy="12" r="3.4" fill="var(--rec)" />
    </svg>
  )
}
